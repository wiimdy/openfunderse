...
cast call "$CLAW_CORE_ADDRESS" \
  "validateIntentExecution(bytes32,(address,address,uint256,uint256,address,bytes))((bool,bool,bool,bool,bool,bool,bytes32,uint64,uint256,bytes32,bytes32))" \
  "$INTENT_HASH" \
  "($TOKEN_IN,$TOKEN_OUT,$AMOUNT_IN,$MIN_AMOUNT_OUT,$ADAPTER,$ADAPTER_DATA)" \
  --rpc-url "$RPC_URL"
