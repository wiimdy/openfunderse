...
test("computeStakeWeightedAggregate: equal stake → simple average", () => {
  ...
  assert.ok(
    result[0] >= SCALE / BigInt(2) - BigInt(1) && result[0] <= SCALE / BigInt(2) + BigInt(1)
  );
  assert.ok(
    result[1] >= SCALE / BigInt(2) - BigInt(1) && result[1] <= SCALE / BigInt(2) + BigInt(1)
  );
});
...
test("computeStakeWeightedAggregate: unequal stake → weighted toward heavier", () => {
  ...
  assert.ok(result[1] >= SCALE / BigInt(4) - BigInt(1) && result[1] <= SCALE / BigInt(4) + BigInt(1));
});
